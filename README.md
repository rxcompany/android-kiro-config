# android-kiro-config

Prizm Android 프로젝트의 Kiro CLI 설정 저장소.

## 왜 분리했나?

프로젝트 repo(`prizm-android`)에 `.kiro/`와 `.amazonq/`가 혼재되어 있었고, 심볼릭 링크 체인과 워크트리별 개별 설정이 복잡했습니다. 이를 해결하기 위해:

- `.kiro/` 설정을 별도 저장소로 분리하여 독립적으로 버전 관리
- 프로젝트 repo의 `.gitignore`에 `.kiro/`를 추가하여 완전 분리
- 워크트리에서는 메인의 `.kiro`를 심볼릭 링크 하나로 연결 (기존 개별 링크 + skip-worktree 방식 제거)
- 팀 공유 설정과 개인 설정을 `.gitignore`와 `private/` 패턴으로 명확히 구분

## 셋업

```bash
# 프로젝트 루트에서 실행
git clone https://github.com/rxcompany/android-kiro-config.git .kiro
```

## 구조

```
.kiro/
├── agents/                 # 에이전트 설정 (전체 개인)
├── settings/               # MCP 서버 등 (전체 개인)
├── skills/                 # 스킬 정의 (팀 공유)
│   ├── */SKILL.md
│   └── private/            # 개인 스킬
├── steering/               # 컨텍스트 (팀 공유)
│   ├── memory-bank/        # 프로젝트 가이드라인, 기술 스택, 도메인 용어
│   ├── dependency-management.md
│   └── private/            # 개인 steering rules
└── private/                # 개인 전용 공간
```

## 스킬/컨텍스트 수정 가이드

`.kiro`는 메인 워크트리에 원본이 있고, 다른 워크트리에서는 심볼릭 링크로 참조합니다. 따라서:

- **어디서 수정해도 동일한 파일을 수정하는 것**입니다 (심볼릭 링크이므로)
- 커밋/푸시도 **어느 워크트리에서든 가능**합니다 (`.kiro/.git/`이 메인에 있고, 심볼릭 링크를 통해 접근)
- IDE의 Git 탭에서 `.kiro` repo 변경사항을 확인하고 커밋하거나, 터미널에서 `cd .kiro && git add -A && git commit && git push`

## 개인 설정

아래 경로들은 `.gitignore`로 제외됩니다. 각자 환경에 맞게 생성하세요.

| 경로 | 용도 |
|------|------|
| `agents/` | 에이전트 설정 (전체 개인) |
| `settings/` | MCP 서버 설정 (전체 개인) |
| `skills/credentials.md` | JIRA, Notion 등 인증 정보 |
| `skills/private/` | 개인 스킬 |
| `steering/private/` | 개인 steering rules |
| `private/` | 최상위 개인 전용 공간 |

## IDE VCS 설정

Android Studio에서 `.kiro/` 내부 변경사항을 Git 탭에서 확인하려면 VCS root를 추가하세요.

`.idea/vcs.xml`:
```xml
<component name="VcsDirectoryMappings">
  <mapping directory="" vcs="Git" />
  <mapping directory="$PROJECT_DIR$/.kiro" vcs="Git" />
</component>
```

이렇게 하면 프로젝트 repo와 `.kiro` repo의 변경사항이 Git 탭에서 각각 분리되어 표시되고, 커밋도 각 repo에 개별적으로 할 수 있습니다.

워크트리에서도 동일하게 `.idea/vcs.xml`에 추가해야 합니다.
vcs.xml은 IDE로 프로젝트를 연 후 생성되므로 수동 추가하거나 kiro에게 명령해야합니다.

## Git credential 설정

HTTPS로 push하려면 gh CLI credential helper 설정이 필요합니다:

```bash
gh auth setup-git
```
